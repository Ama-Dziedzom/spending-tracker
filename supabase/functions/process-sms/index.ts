...
serve(async (req) => {
  ...
  try {
    ...
    // ── Auth: derive userId from JWT (required). ──────────────────────────────
    // The Share Extension sends a JWT directly. The iOS Shortcut must also send
    // a JWT — update it to include: Authorization: Bearer <token>.
    // The `secret` field is an optional extra check for Shortcut automation.
    ...
    if (!userRes.ok) {
      return new Response(
        JSON.stringify({ error: 'Unauthorized — include your Supabase JWT in the Authorization header' }),
    ...
    if (!userData?.id) {
      return new Response(
        JSON.stringify({ error: 'Unauthorized — could not identify user from token' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    const userId: string = userData.id

    // Optional: if SHORTCUT_SECRET is configured and a secret was provided,
    // it must match. This lets the iOS Shortcut add a second factor without
    // requiring a secret for the Share Extension (which uses JWT only).
    const SHORTCUT_SECRET = Deno.env.get('SHORTCUT_SECRET')
    if (SHORTCUT_SECRET && secret && secret !== SHORTCUT_SECRET) {
      return new Response(
    ...
    // ── Parse via parse-sms function (no service role key used there) ─────────
    ...
    // ── STEP C: Update account balance — SNAPSHOT SYNC (unchanged logic) ──────
